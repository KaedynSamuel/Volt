...
const settingsSections = [
  ...
  { id: "billing", title: "Billing", description: "Banking and subscription details", icon: CreditCard },
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedSettings) {
        ...
        if (parsed.billing) setBilling({ ...defaultBilling, ...parsed.billing })
  ...
  return (
    <DashboardLayout title="Settings" subtitle="Manage Volt account, security, appearance, billing, and support preferences.">
      <style jsx global>{`
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <main className="flex-1 rounded-2xl border border-border bg-card/70 p-5 shadow-lg backdrop-blur-xl">
          ...
          {activeSection === "billing" && (
            <section className="max-w-3xl">
              <div className="mb-6">
                <span className="mb-3 inline-flex rounded-full border border-[#d011c9]/30 bg-[#d011c9]/10 px-3 py-1 text-xs font-semibold text-[#d011c9]">Billing</span>
                <h2 className="text-2xl font-bold text-foreground">Banking Details</h2>
                <p className="mt-1 text-sm text-muted-foreground">Update customer billing and bank information for the Volt subscription.</p>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <input value={billing.accountHolder} onChange={(event) => updateBillingField("accountHolder", event.target.value)} placeholder="Account holder" className="h-11 rounded-xl border border-border bg-input px-3 text-sm outline-none" />
                <input value={billing.bankName} onChange={(event) => updateBillingField("bankName", event.target.value)} placeholder="Bank name" className="h-11 rounded-xl border border-border bg-input px-3 text-sm outline-none" />
                <input value={billing.accountNumber} onChange={(event) => updateBillingField("accountNumber", event.target.value)} placeholder="Account number" className="h-11 rounded-xl border border-border bg-input px-3 text-sm outline-none" />
                <input value={billing.branchCode} onChange={(event) => updateBillingField("branchCode", event.target.value)} placeholder="Branch code" className="h-11 rounded-xl border border-border bg-input px-3 text-sm outline-none" />
              </div>
              <div className="mt-5 rounded-2xl border border-border bg-background/40 p-4">
                <div className="flex items-center gap-3">
                  <WalletCards className="h-5 w-5 text-[#05a391]" />
                  <div>
                    <p className="text-sm font-semibold">Volt Subscription Billing</p>
                    <p className="text-xs text-muted-foreground">Use this area for normal subscription billing and payment method updates.</p>
                  </div>
                </div>
              </div>
              <button type="button" onClick={() => saveUserSettings("Billing details saved locally for now. Connect this to your payment backend when ready.")} className="mt-5 h-10 rounded-xl bg-gradient-to-r from-[#d011c9] to-[#05a391] px-4 text-sm font-semibold text-white">Save Billing</button>
            </section>
          )}
